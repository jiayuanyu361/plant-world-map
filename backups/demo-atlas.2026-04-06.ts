...
export function withPlantImage(plant: Plant): Plant {
  ...
  return {
    ...
    tags: mergeTags(plant.tags ?? [], meta?.season)
  };
}
...
export function getPlantInsights(name: string) {
  const meta = getPlantMeta(name);
  if (!meta) {
    return null;
  }
  ...
  return {
    season: meta.season,
    viewingSeason: meta.viewingSeason,
    regionFocus: meta.regionFocus,
    habitat: meta.habitat,
    flowerMeaning: meta.flowerMeaning
  };
}
...
function mergeTags(tags: string[], season?: Season) {
